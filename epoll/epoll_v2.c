// epoll 实现单线程 reactor echo server
...
int recv_callback(int fd, int events, void *arg)
{
    ...
    else if (ret == 0) // 客户端关闭连接， 或者 events & EPOLLRDHUP > 0
    {
        printf("Client closed: %d\n", fd);
        evt.events = EPOLLIN;
        epoll_ctl(event_loop->ep_fd, EPOLL_CTL_DEL, fd, &evt);
        close(fd); // 直接关闭连接， 发送 FIN 给客户端
        // shutdown(fd, SHUT_RDWR); // 断开读和写通道, 发送 FIN 给客户端， 断开连接, 等同于 close
        free(si); // 记得释放内存

        // 如果要支持半连接
        // shutdown(fd, SHUT_RD); // 关闭读通道， 不发送数据， 只把 socket 标记为 RCV_SHUTDOWN
        // send(fd, buffer, buf_size, 0);  // 把服务端剩下的数据发给客户端
        // close(fd); // 关闭连接
    }
    else // 正常读取到数据
    {
